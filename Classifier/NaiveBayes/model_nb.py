...
from sklearn.naive_bayes import GaussianNB  # Changed to Naive Bayes
...
def train_and_evaluate_model(df, output_path):
    """
    Trains a Naive Bayes model, evaluates it, and saves the results.
    """
    # --- 2. DATA PREPARATION ---
    ...
    # Note: GaussianNB assumes normal distribution, but MinMax scaling ensures
    # all features are on a similar scale, which is generally good practice.
    scaler = MinMaxScaler()
    ...
    # GaussianNB is chosen because the features include continuous variables.
    model = GaussianNB()
    ...
    model_path = os.path.join(os.getcwd(), 'naive_bayes_model.joblib')  # Renamed file
    ...
    with open(metrics_path, 'w') as f:
        f.write("Model Evaluation Metrics (Naive Bayes)\n")
        f.write("--------------------------------------\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train a Naive Bayes model for document element classification.")
    ...
    try:
        loaded_model = joblib.load('naive_bayes_model.joblib')  # Renamed file
