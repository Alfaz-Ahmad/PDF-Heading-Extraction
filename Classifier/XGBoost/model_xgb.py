...
column_map = {
    'text': 'text',
    'font': 'font',
    'normalised_font_size_rank': 'norm_font_size_rank',  # Corrected to match your CSV
...
def train_and_evaluate_model(df, output_path):
    """
    Trains an XGBoost model, evaluates it, and saves the results.
    """
    # --- 2. DATA PREPARATION ---
    # Separate features (X) and target variable (y)
    # Perform a document-aware train-test split to prevent data leakage.
    # We will split the unique document IDs, not the individual rows.
    ...
    model_path = os.path.join(os.getcwd(), 'xgboost_model.joblib')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train an XGBoost model for document element classification.")
